...
SELECT 
    ...
    CONCAT('{"consultationId": ', c.id, ', "expertId": ', c.expertId, '}') as metadata,
    NOW() as processedAt,
    NOW() as createdAt,
    NOW() as updatedAt
FROM consultations c
...
-- 7. 결제 수단 데이터 (payment_methods) - 올바른 구조로 수정
INSERT IGNORE INTO payment_methods (userId, type, name, last4, bankName, isDefault, expiryDate, createdAt, updatedAt)
SELECT 
    u.id as userId,
    'credit_card' as type,
    CONCAT(u.name, '의 카드') as name,
    LPAD(FLOOR(RAND() * 10000), 4, '0') as last4,
    '신한은행' as bankName,
    1 as isDefault,
