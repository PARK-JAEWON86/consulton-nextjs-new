-- 안전한 데이터 삽입 (기존 데이터가 있는 테이블은 건너뛰기)

-- 1. AI 사용량 데이터 (ai_usages) - 기존 데이터가 없을 때만 삽입
...
-- 2. 전문가 가용성 데이터 (expert_availability) - 기존 데이터가 없을 때만 삽입
INSERT IGNORE INTO expert_availability (expertId, dayOfWeek, startTime, endTime, isAvailable, createdAt, updatedAt)
SELECT 
    e.id as expertId,
    'monday' as dayOfWeek,
...
-- 3. 상담 세션 데이터 (consultation_sessions) - 기존 데이터가 없을 때만 삽입
INSERT IGNORE INTO consultation_sessions (consultationId, sessionType, startTime, endTime, duration, status, notes, createdAt, updatedAt)
SELECT 
    c.id as consultationId,
    'video' as sessionType,
...
-- 4. 상담 요약 데이터 (consultation_summaries) - 기존 데이터가 없을 때만 삽입
INSERT IGNORE INTO consultation_summaries (consultationId, summary, keyPoints, recommendations, followUpActions, createdAt, updatedAt)
SELECT 
    c.id as consultationId,
    '상담 요약: 고객의 문제를 분석하고 해결방안을 제시했습니다.' as summary,
    '["문제 분석", "해결방안 제시", "후속 조치 계획"]' as keyPoints,
    '["정기적인 상담 예약", "자기계발 활동", "건강한 생활습관 유지"]' as recommendations,
    '["1주일 후 재상담", "과제 수행 확인", "진행상황 점검"]' as followUpActions,
    NOW() as createdAt,
    NOW() as updatedAt
FROM consultations c
WHERE c.status = 'completed'
...
-- 5. 결제 내역 데이터 (payments) - 기존 데이터가 없을 때만 삽입
INSERT IGNORE INTO payments (userId, expertId, consultationId, amount, currency, paymentMethod, status, transactionId, createdAt, updatedAt)
SELECT 
    c.userId,
    c.expertId,
    c.id as consultationId,
...
-- 6. 알림 데이터 (notifications) - 기존 데이터가 없을 때만 삽입
INSERT IGNORE INTO notifications (userId, type, title, message, isRead, createdAt, updatedAt)
SELECT 
    u.id as userId,
...
-- 7. 결제 수단 데이터 (payment_methods) - 기존 데이터가 없을 때만 삽입
