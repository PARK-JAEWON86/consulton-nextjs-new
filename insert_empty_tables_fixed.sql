-- 수정된 데이터 삽입 SQL (실제 테이블 구조에 맞춤)

-- 1. AI 사용량 데이터 (ai_usages) - 기본 구조로 수정
...
-- 2. 전문가 가용성 데이터 (expert_availability) - 기본 구조로 수정
-- 주의: 실제 테이블 구조에 따라 컬럼명이 다를 수 있습니다
INSERT IGNORE INTO expert_availability (expertId, dayOfWeek, isAvailable, createdAt, updatedAt)
SELECT 
    e.id as expertId,
    'monday' as dayOfWeek,
    1 as isAvailable,
...
-- 3. 상담 세션 데이터 (consultation_sessions) - 기본 구조로 수정
...
-- 4. 상담 요약 데이터 (consultation_summaries) - 기본 구조로 수정
...
-- 5. 결제 내역 데이터 (payments) - 기본 구조로 수정
...
-- 6. 알림 데이터 (notifications) - 기본 구조로 수정
...
-- 7. 결제 수단 데이터 (payment_methods) - 기본 구조로 수정
